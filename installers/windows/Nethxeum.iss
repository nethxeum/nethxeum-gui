; Nethxeum Helium Synthetic GUI Wallet Installer for Windows
; Copyright (c) 2017-2024, The Monero Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\nethxeum-wallet-gui.exe")

[Setup]
AppName=Nethxeum GUI Wallet
...
DefaultDirName={commonpf}\Nethxeum GUI Wallet
DefaultGroupName=Nethxeum GUI Wallet
...
LicenseFile=LICENSE
AppPublisher=The Nethxeum Developer Community
AppPublisherURL=https://nethxeum.com
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Nethxeum files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Nethxeum.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Nethxeum GUI wallet exe and guide
Source: "bin\nethxeum-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\nethxeum-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Nethxeum CLI wallet
Source: "bin\extras\nethxeum-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Nethxeum wallet RPC interface implementation
Source: "bin\extras\nethxeum-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Nethxeum daemon
Source: "bin\nethxeumd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Nethxeum daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "nethxeum-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Nethxeum blockchain utilities
Source: "bin\extras\nethxeum-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\nethxeum-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\nethxeum-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitnethxeum');
  s := 'The default folder to store the Nethxeum blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\nethxeum\nethxeum-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Nethxeum default location
...
begin
  Result := BlockChainDir('') + '\bitnethxeum.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "nethxeum-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Nethxeum daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\nethxeumd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\nethxeum-daemon.bat'), s, false); 
  end;
end;
...
Name: "{group}\Utilities\Nethxeum Daemon"; Filename: "{app}\nethxeumd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\nethxeum-wallet-cli.exe"; WorkingDir: "{userdocs}\Nethxeum\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Nethxeum\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\nethxeum-wallet-gui\nethxeum-wallet-gui.log"""
...
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM nethxeumd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\nethxeum-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\nethxeum"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\nethxeum\nethxeum-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\nethxeum\nethxeum-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "nethxeum:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "nethxeum://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "nethxeum"; ValueType: "string"; ValueData: "URL:Nethxeum Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "nethxeum"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "nethxeum\DefaultIcon"; ValueType: "string"; ValueData: "{app}\nethxeum-wallet-gui.exe,0"
Root: HKCR; Subkey: "nethxeum\shell\open\command"; ValueType: "string"; ValueData: """{app}\nethxeum-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "nethxeumseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "nethxeumseed://a.b.c.d:port"
Root: HKCR; Subkey: "nethxeumseed"; ValueType: "string"; ValueData: "URL:Nethxeum Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "nethxeumseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "nethxeumseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\nethxeum-wallet-gui.exe,0"
Root: HKCR; Subkey: "nethxeumseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\nethxeum-wallet-gui.exe"" ""%1"""
