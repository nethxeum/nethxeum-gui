...
gsigs = 'https://github.com/nethxeum/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('nethxeum'):
        # Remove the potentially stale nethxeum dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'nethxeum', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'nethxeum'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/nethxeum/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'nethxeum='+args.commit, '--url', 'nethxeum='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/nethxeum-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/nethxeum/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/nethxeum/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/nethxeum/nethxeum', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/nethxeum', exist_ok=True)
    os.chdir('builder/inputs/nethxeum')
    if args.pull:
