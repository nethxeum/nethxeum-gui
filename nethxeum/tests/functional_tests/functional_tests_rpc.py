...
# a main offline nethxeumd, does most of the tests
# a restricted RPC nethxeumd setup with RPC payment
# two local online nethxeumds connected to each other
N_nethxeumDS = 4

# 4 wallets connected to the main offline nethxeumd
# 1 wallet connected to the first local online nethxeumd
# 1 offline wallet
N_WALLETS = 6
...
nethxeumd_base = [builddir + "/bin/nethxeumd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--p2p-bind-port", "nethxeumd_p2p_port", "--rpc-bind-port", "nethxeumd_rpc_port", "--zmq-rpc-bind-port", "nethxeumd_zmq_port", "--zmq-pub", "nethxeumd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "nethxeumd_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
nethxeumd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/nethxeum-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_nethxeumDS):
  command_lines.append([str(18180+i) if x == "nethxeumd_rpc_port" else str(18280+i) if x == "nethxeumd_p2p_port" else str(18380+i) if x == "nethxeumd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "nethxeumd_zmq_pub" else builddir + "/functional-tests-directory/nethxeumd" + str(i) if x == "nethxeumd_data_dir" else x for x in nethxeumd_base])
  if i < len(nethxeumd_extra):
    command_lines[-1] += nethxeumd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/nethxeumd' + str(i) + '.log', 'a+'))
