...
Core tests take longer than any other nethxeum tests, due to the high amount of computational work involved in validating core components.
...
To run only nethxeum's core tests (after building):
...
To run only nethxeum's crypto tests (after building):
...
```bash
nethxeumd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `nethxeum-wallet-rpc` with RPC port 18330. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only nethxeum's hash tests (after building):
...
To run only nethxeum's performance tests (after building):
...
To run only nethxeum's unit tests (after building):
