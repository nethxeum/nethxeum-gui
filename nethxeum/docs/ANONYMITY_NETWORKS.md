# Anonymity Networks with nethxeum

Currently only Tor and I2P have been integrated into nethxeum. The usage of
...
is made or until `nethxeumd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `nethxeum-wallet-cli` and
`nethxeum-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `nethxeumd` the ip/port of a socks proxy provided by a
...
which tells `nethxeumd` that ".onion" p2p addresses can be forwarded to a socks
...
```
--add-exclusive-node rveahdfho7wo4b2m.onion:28330
--add-peer rveahdfho7wo4b2m.onion:28330
```
...
`--anonymous-inbound`. This option tells `nethxeumd` the inbound address, network
type, and max connections:

```
--anonymous-inbound rveahdfho7wo4b2m.onion:28330,127.0.0.1:28330,25
--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p,127.0.0.1:30000
```

which tells `nethxeumd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28330" and forwarded to `nethxeumd`
localhost port 28330, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `nethxeumd` localhost port 30000.
...
`nethxeumd` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 18331 for mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/nethxeum
...
```
HiddenServiceDir /var/lib/tor/data/nethxeum
HiddenServicePort 28330 127.0.0.1:28330
```

This will store key information in `/var/lib/tor/data/nethxeum` and will forward
"Tor port" 28330 to port 28330 of ip 127.0.0.1. The file
`/usr/lib/tor/data/nethxeum/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the nethxeum network should have
similar timestamps.
...
### Intermittent nethxeum Syncing

If a user only runs `nethxeumd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `nethxeumd` as often as possible to conceal when transactions are being sent.
...
`nethxeumd` currently selects two outgoing connections every 5 minutes for
...
`nethxeumd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `nethxeumd` should likely disconnect
