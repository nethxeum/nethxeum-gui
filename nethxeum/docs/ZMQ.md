# The Current/Future Status of ZMQ in nethxeum

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. nethxeum
...
matching supported by both nethxeum and ZMQ. The `format`, `context` and `event`
...
The nethxeum daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The nethxeum daemon also ensures that
...
The nethxeum daemon will send a `txpool_add` pub exactly once for each
