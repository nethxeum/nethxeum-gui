...
3. `git clone --recursive -b v0.X.Y.Z --depth 1 https://github.com/nethxeum/nethxeum-gui` 

4. Compile `nethxeum-wallet-gui.app`
...
5. Replace the `nethxeumd` binary inside `nethxeum-wallet-gui.app/Contents/MacOS/` with one built using deterministic builds / gitian.
...
2. `codesign --deep --force --verify --verbose --options runtime --timestamp --entitlements entitlements.plist --sign 'XXXXXXXXXX' nethxeum-wallet-gui.app`

You can check if this step worked by using `codesign -dvvv nethxeum-wallet-gui.app`

3. `hdiutil create -fs HFS+ -srcfolder nethxeum-gui-v0.X.Y.Z -volname nethxeum-wallet-gui nethxeum-gui-mac-x64-v0.X.Y.Z.dmg`

4. `xcrun notarytool submit nethxeum-gui-mac-x64-v0.X.Y.Z.dmg --apple-id email@address.org --team-id XXXXXXXXXX`

5. `xcrun notarytool info aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeee --apple-id email@address.org --team-id XXXXXXXXXX`

6. `xcrun stapler staple -v nethxeum-gui-mac-x64-v0.X.Y.Z.dmg`
...
The `CMAKE_PREFIX_PATH` has to be set to `/path/to/qt-build-dir/` during nethxeum-gui compilation.
